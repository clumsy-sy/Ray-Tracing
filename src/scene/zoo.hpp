...
auto cornell_box_bunny_rotate(hittable_list &world, hittable_list &light) -> void {
  ...
  auto m = nullptr;
  light.add(std::make_unique<quad>(point3(343, 554, 232), vec3d(-130, 0, 0), vec3d(0, 0, -105), m));
  world.add(std::make_unique<quad>(point3(343, 554, 232), vec3d(-130, 0, 0), vec3d(0, 0, -105), wlight));
  ...
  // auto box1 = std::make_unique<translate>(
  //     std::make_unique<rotate_y>(std::make_unique<box>(point3(0, 0, 0), point3(165, 330, 165), aluminum), 15),
  //     vec3d(265, 0, 295));
  // world.add(std::move(box1));
